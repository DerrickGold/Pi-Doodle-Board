...
nj_result_t njReadHeader(const void *jpeg, const int size){
    njDone();
    nj.pos = (const unsigned char*) jpeg;
    nj.size = size & 0x7FFFFFFF;
    if ((nj.pos[0] ^ 0xFF) | (nj.pos[1] ^ 0xD8)) return NJ_NO_JPEG;
    ...
    while (!nj.error) {
        njSkip(2);
        switch (nj.pos[-1]) {
            case 0xC0: njDecodeSOF();  return NJ_OK; break;
            case 0xC4: njSkipMarker();  break;
            case 0xDB: njSkipMarker();  break;
            case 0xDD: njSkipMarker();  break;
            case 0xDA: njSkipMarker(); break;
            case 0xFE: njSkipMarker(); break;
            default:
                if ((nj.pos[-1] & 0xF0) == 0xE0)
                    njSkipMarker();
