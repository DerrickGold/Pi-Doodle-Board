...
static inline void _png_fillBuffer(GFXObj_t *object, const u8 *image){
	...
	do{
		...
		do{
			...
			r = (image[ pos + 0]>>3) & 31; /*red*/
			g = (image[ pos + 1]>>2) & 63;/*green*/
			b = (image[ pos+ 2]>>3) & 31;  /*blue*/
...
int BAG_Display_LoadPng(const char *filename,  GFXObj_t *object){
	int err = ERR_NONE;
	u8* buffer = NULL;
	u8* image = NULL;
	...
	if(!LodePNG_loadFile ( &buffer, &buffersize, filename )){
		...
		int BmpWd = pngDecoder.infoPng.width,
        BmpHt = pngDecoder.infoPng.height;

		//start the conversion
		err = BAG_Display_CreateObj(object, 32, BmpWd, BmpHt, BmpWd, BmpHt);
		if(err != ERR_NONE) goto end;

		err = BAG_Display_CreateGfxAlphaBuf(object);
		if(err != ERR_NONE) goto end;
